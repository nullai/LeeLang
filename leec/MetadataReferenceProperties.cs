...
namespace leec
{
	...
	public struct MetadataReferenceProperties : IEquatable<MetadataReferenceProperties>
	{
		...
		public MetadataReferenceProperties(MetadataImageKind kind = MetadataImageKind.Assembly, string[] aliases = default(string[]), bool embedInteropTypes = false)
		{
			if (!kind.IsValid())
			{
		...
		public MetadataReferenceProperties(MetadataImageKind kind, string[] aliases, bool embedInteropTypes, bool hasRecursiveAliases)
		...
		{
			return WithAliases(aliases.ToArray());
		}
		...
		/// <see cref="Kind"/> is <see cref="MetadataImageKind.Module"/>, as modules can't be aliased.
		/// </exception>
		public MetadataReferenceProperties WithAliases(string[] aliases)
		{
