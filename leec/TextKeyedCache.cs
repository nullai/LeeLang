...
namespace leec
{
	public class TextKeyedCache<T> where T : class
	{
		...
		private readonly LocalTableItem[] _localTable = new LocalTableItem[LocalSize];
		...
		private static readonly SharedTableItem[] s_sharedTable = new SharedTableItem[SharedSize];

		// store a reference to shared cache locally
		// accessing a static field of a generic type could be nontrivial
		private readonly SharedTableItem[] _sharedTableInst = s_sharedTable;
