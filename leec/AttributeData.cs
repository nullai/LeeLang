...
namespace leec
{
	...
	public abstract class AttributeData
	{
		...
		public GreenNode ApplicationSyntaxReference { get { return CommonApplicationSyntaxReference; } }
		protected abstract GreenNode CommonApplicationSyntaxReference { get; }

		/// <summary>
		/// Constructor arguments on the attribute.
		/// </summary>
		public TypedConstant[] ConstructorArguments { get { return CommonConstructorArguments; } }
		public abstract TypedConstant[] CommonConstructorArguments { get; }

		/// <summary>
		/// Named (property value) arguments on the attribute. 
		/// </summary>
		public KeyValuePair<string, TypedConstant>[] NamedArguments { get { return CommonNamedArguments; } }
		public abstract KeyValuePair<string, TypedConstant>[] CommonNamedArguments { get; }
		...
		public static bool IsTargetEarlyAttribute(INamedTypeSymbol attributeType, int attributeArgCount, AttributeDescription description)
		{
			...
			for (int i = 0; i < attributeCtorsCount; i++)
			{
				...
				{
					string actualNamespaceName = attributeType.ContainingNamespace.ToDisplayString();
		...
		private static T DecodeNamedArgument<T>(KeyValuePair<string, TypedConstant>[] namedArguments, string name, SpecialType specialType, T defaultValue = default(T))
		{
		...
		private static int IndexOfNamedArgument(KeyValuePair<string, TypedConstant>[] namedArguments, string name)
		{
			// For user defined attributes VB allows duplicate named arguments and uses the last value.
			// Dev11 reports an error for pseudo-custom attributes when emitting metadata. We don't.
		...
		public ConstantValue DecodeDecimalConstantValue()
		{
			...
			TypedConstant[] args = this.CommonConstructorArguments;
		...
		public static AttributeUsageInfo DecodeAttributeUsageAttribute(TypedConstant positionalArg, KeyValuePair<string, TypedConstant>[] namedArgs)
		{
			// BREAKING CHANGE (C#):
