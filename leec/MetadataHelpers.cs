...
namespace leec
{
	public static class MetadataHelpers
	{
		...
		private struct SerializedTypeDecoder
		{
			...
			private static void HandleDecodedTypeName(string decodedTypeName, bool decodingTopLevelType, ref string topLevelType, ref List<string> nestedTypesBuilder)
			{
		...
		public static string GetAritySuffix(int arity)
		{
			...
			return (arity <= 9) ? s_aritySuffixesOneToNine[arity - 1] : string.Concat(GenericTypeNameManglingString, arity.ToString());
		}
		...
		/// </summary>
		private static readonly string[] s_splitQualifiedNameSystem = { };
			  ...
			  string name)
		{
			...
			if (dots == 0)
			{
				return name == SystemString ? s_splitQualifiedNameSystem : new string[] { name};
			}

			var result = new List<string>(dots + 1);
		...
		*/
		...
		public static void CheckAssemblyOrModuleName(string name, CommonMessageProvider messageProvider, int code, List<Diagnostic> builder)
		{
			string errorArgumentResourceId = GetAssemblyOrModuleNameErrorArgumentResourceName(name);
			if (errorArgumentResourceId != null)
			{
