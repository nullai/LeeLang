...
namespace leec
{
	public class DirectiveParser : SyntaxParser
	{
		...
		public LeeSyntaxNode ParseDirective(
			bool isActive,
			bool endIsActive,
		...
		private DirectiveTriviaSyntax ParseLineDirective(SyntaxToken hash, SyntaxToken id, bool isActive)
		{
			...
			switch (this.CurrentToken.Kind)
			{
				...
				default:
					...
					{
						if ((int)line.GetValue() < 1)
						{
							line = this.AddError(line, ErrorCode.ERR_InvalidLineNumber);
						}
						else if ((int)line.GetValue() > 0xfeefed)
						{
		...
		private object Evaluate(ExpressionSyntax expr)
		{
			switch (expr.Kind)
			{
				...
				case SyntaxKind.FalseLiteralExpression:
					return ((LiteralExpressionSyntax)expr).Token.GetValue();
				...
				case SyntaxKind.IdentifierName:
					...
					string id = ((IdentifierNameSyntax)expr).Identifier.GetValueText();
					bool constantValue;
