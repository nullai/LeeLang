...
namespace leec
{
	public sealed class ModuleMetadata : Metadata
	{
		...
		public static ModuleMetadata CreateFromMetadata(IntPtr metadata, int size)
		{
			...
			if (size <= 0)
			{
				throw new ArgumentOutOfRangeException("@SizeHasToBePositive", nameof(size));
			}

			return new ModuleMetadata(metadata, size, includeEmbeddedInteropTypes: false, ignoreAssemblyRefs: false);
		}

		public static ModuleMetadata CreateFromMetadata(IntPtr metadata, int size, bool includeEmbeddedInteropTypes, bool ignoreAssemblyRefs = false)
		{
		...
		public static ModuleMetadata CreateFromImage(IntPtr peImage, int size)
		{
			...
			return new ModuleMetadata(/*new PEReader((byte*)peImage, size)*/null as PEReader);
		}
		...
		public static ModuleMetadata CreateFromImage(IEnumerable<byte> peImage)
		{
			...
			return CreateFromImage(peImage.ToArray());
		}
		...
		/// <exception cref="ArgumentNullException"><paramref name="peImage"/> is null.</exception>
		public static ModuleMetadata CreateFromImage(byte[] peImage)
		{
			if (peImage == null)
			{
				throw new ArgumentNullException(nameof(peImage));
			...
			return new ModuleMetadata(/*new PEReader(peImage)*/null as PEReader);
		}
		...
		public static ModuleMetadata CreateFromStream(Stream peStream, PEStreamOptions options)
		{
			...
			return new ModuleMetadata(/*new PEReader(peStream, options)*/null as PEReader);
		}
