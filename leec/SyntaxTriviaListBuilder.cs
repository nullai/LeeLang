	...
	public class SyntaxTriviaListBuilder
	{
		...
		public SyntaxTriviaList ToList()
		{
			if (_count > 0)
			{
				...
				{
					case 1:
						return new SyntaxTriviaList(default(SyntaxToken), _nodes[0], position: 0, index: 0);
					case 2:
						return new SyntaxTriviaList(default(SyntaxToken),
							SyntaxList.List(
							_nodes[0],
							_nodes[1]), position: 0, index: 0);
					case 3:
						return new SyntaxTriviaList(default(SyntaxToken),
							SyntaxList.List(
								_nodes[0],
					...
					default:
						{
							...
							return new SyntaxTriviaList(default(SyntaxToken),
								SyntaxList.List(tmp), position: 0, index: 0);
						}
				}
			}
			else
