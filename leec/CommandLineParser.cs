...
namespace leec
{
	public abstract class CommandLineParser
	{
		...
		public CommandLineParser(CommonMessageProvider messageProvider)
		{
		...
		protected KeyValuePair<string, string>[] ParsePathMap(string pathMap, IList<Diagnostic> errors)
		{
			var pathMapBuilder = new List<KeyValuePair<string, string>>();
			if (string.IsNullOrEmpty(pathMap))
			{
				return pathMapBuilder.ToArray();
			}
			...
			out string errorMessage)
		{
			...
			foreach (var arg in args)
			{
				...
				if (isClientArgsOption(arg, "keepalive", out hasValue, out value))
				{
					...
					if (int.TryParse(value, out intValue))
					{
						if (intValue < -1)
						{
							errorMessage = "@KeepAliveIsTooSmall";
							return false;
						}
					...
					{
						errorMessage = "@KeepAliveIsNotAnInteger";
						return false;
					}
					continue;
				}
				...
				if (isClientArgsOption(arg, "shared", out hasValue, out value))
				{
					if (hasValue)
					{
						if (string.IsNullOrEmpty(value))
						{
							errorMessage = "@SharedArgumentMissing";
							return false;
						}
			...
			{
				errorMessage = "@KeepAliveWithoutShared";
				return false;
			}
			else
			{
		...
		public static string MismatchedVersionErrorText => "@MismatchedVersion";
		...
		private CommandLineSourceFile ToCommandLineSourceFile(string resolvedPath)
		{
			...
			return new CommandLineSourceFile(resolvedPath);
		}
		...
		public IEnumerable<CommandLineSourceFile> ParseSeparatedFileArgument(string value, string baseDirectory, IList<Diagnostic> errors)
		{
