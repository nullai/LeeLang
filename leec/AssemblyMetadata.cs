...
namespace leec
{
	public sealed class AssemblyMetadata : Metadata
	{
		private sealed class Data
		{
			...
			public readonly ModuleMetadata[] Modules;
			...
			public Data(ModuleMetadata[] modules, PEAssembly assembly)
			{
		...
		/// </summary>
		private readonly ModuleMetadata[] _initialModules;
		...
		private ModuleMetadata[] _lazyPublishedModules;
		...
		/// </remarks>
		public readonly List<IAssemblySymbol> CachedSymbols = new List<IAssemblySymbol>();
		...
		public static AssemblyMetadata CreateFromImage(byte[] peImage)
		{
		...
		public static AssemblyMetadata Create(ModuleMetadata module)
		{
			...
			return new AssemblyMetadata(new ModuleMetadata[] { module });
		}
		...
		public static AssemblyMetadata Create(ModuleMetadata[] modules)
		{
			...
			{
				throw new ArgumentException("@AssemblyMustHaveAtLeastOneModule", nameof(modules));
			}
			...
			for (int i = 0; i < modules.Length; i++)
			{
				...
				{
					throw new ArgumentException("@ModuleCopyCannotBeUsedToCreateAssemblyMetadata", nameof(modules) + "[" + i + "]");
				}
			}
		...
		public ModuleMetadata[] GetModules()
		{
			if (_lazyPublishedModules == null)
			{
				...
				if (!IsImageOwner)
				{
					newModules = new ModuleMetadata[newModules.Length];
					for (int i = 0; i < newModules.Length; i++)
						newModules[i] = data.Modules[i].Copy();
				}

				_lazyPublishedModules = newModules;
			}
